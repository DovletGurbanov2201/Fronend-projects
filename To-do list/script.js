function createCards() {
    ...
    for (let i = 0; i < localStorage.length; i++) {
        ...
        if (event_params_arr[2] != "" && event_params_arr[3] != "") {
            card.innerHTML = 
            `
                ...
                <div class="card-header" style = "background-color: red;">Name of the event:&nbsp &nbsp ${event_params_arr[0]}</div>
                    <ul class="list-group list-group-flush">
                        <li class="list-group-item" id="${i}">Date and time: ${upgradeTime(timeStr)}</li>
                        ...
                        <li class="list-group-item"><button onclick="deleteEvent(${i})" class="delete-btn">Delete this event</button></li>
                    </ul>
                </div>  
            `
            EventsContainer.append(card)
        }
        else if(event_params_arr[2] == ""){
            card.innerHTML = 
            `
                <div style="width: 18rem;" class="card>
                <div class="card-header" style = "background-color: red;">Name of the event:&nbsp &nbsp ${event_params_arr[0]}</div>
